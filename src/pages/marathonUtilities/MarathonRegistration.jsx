...
export default function MarathonRegistration() {
  ...
  const marathonRegistration = (e) => {
    ...
    const Fname = form.fname.value; const Lname= form.lname.value; const Name=(Fname+' '+Lname);
    const Phone = form.phone.value; const Gender = form.gender.value; const Blood = form.blood_group.value;
    const applyTime = new Date();

    const applicationDetails = {Email : user.email, field_Id : _id , submitionTime : applyTime, Name, Phone, Gender, Blood};
    fetch(`http://localhost:5000/apply-marathon`, {
      method: "POST",
    ...
    <div className="  mx-auto w-11/12">
      <div className="grid grid-cols-1 md:grid-cols-2 justify-center items-center">
        ...
        >
          ...
          <div className="text-start font-bold text-xl">Start Date: <span className="text-xl font-extralight">{marathonDate.split("T")[0]}</span></div>
          ...
          <button className="text-center  hover:btn-outline  items-center col-span-1 md:col-span-2 bg-slate-400 py-2 mt-3" onClick={marathonRegistration}>
            Register
          </button>
        </form>
      </div>
